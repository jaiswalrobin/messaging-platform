...
- Server→client: `message_sent {messageId, conversationId, clientMessageId, status:'sent'}` (to sender), `message_received {messageId, conversationId, senderId, content, createdAt}` (to recipients), `message_delivered {messageId, conversationId, recipientId, deliveredAt, status:'delivered', allDelivered}` (to the sender's user node — its only job is upgrading the sender's outgoing tick; `allDelivered` is the server-side verdict that everyone acked), `message_read {conversationId, readerId, lastReadMessageId, readAt, status:'read'}` (to everyone but the reader), `error {code: FORBIDDEN|PERSIST_FAILED|FETCH_FAILED|INTERNAL, ...}` (`PERSIST_FAILED` = the message was not persisted — direct-fallback or DLQ final-skip).
...
- Delivery receipts (gray ✓✓): the recipient's client sends `message_delivered {conversationId, messageId, senderId}` only after actually processing the frame (app-level ack — a throttled/DevTools-offline browser never acks) → consumer persists to `message_receipts` (deduped read-before-write) → re-reads receipts to compute `allDelivered` (recipients = everyone but the sender, all of whom acked) → broadcasts `message_delivered` **only to the sender's user node** (all sender devices; other participants have no tick to upgrade). Sender-side upgrade is monotonic (never downgrades read), and gated on `allDelivered` — first acks in a group don't flip the tick.
